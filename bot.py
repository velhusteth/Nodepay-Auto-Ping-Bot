...
class Nodepay:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}Velhust Auto Ping {Fore.BLUE + Style.BRIGHT}Nodepay
            """
            f"""
        {Fore.GREEN + Style.BRIGHT}Chúc Mọi Người Một Ngày Tốt Lành {Fore.YELLOW + Style.BRIGHT}From Velhust With Love !
            """
        )
    ...
    def print_question(self):
        while True:
            try:
                print(f"{Fore.WHITE + Style.BRIGHT}1. Chạy với Proxy Free Proxyscrape{Style.RESET_ALL}")
                ...
                if choose in [1, 2, 3]:
                    proxy_type = (
                        "với Proxy Free Proxyscrape" if choose == 1 else 
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 401:
                    self.print_session_message(email, proxy, Fore.RED, f"Lấy Session Thất bại: {Fore.YELLOW + Style.BRIGHT}Np Token đã hết hạn{Style.RESET_ALL}")
                    return None
                response.raise_for_status()
                ...
                if attempt < retries - 1:
                    ...
                    continue
                self.print_session_message(email, proxy, Fore.RED, f"Lấy Session Thất bại: {Fore.YELLOW + Style.BRIGHT}{str(e)}{Style.RESET_ALL}")
    ...
    async def mission_lists(self, email: str, proxy=None, retries=5):
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 401:
                    self.print_session_message(email, proxy, Fore.RED, f"Lấy Danh sách Nhiệm vụ Thất bại: {Fore.YELLOW + Style.BRIGHT}Np Token đã hết hạn{Style.RESET_ALL}")
                    return None
                response.raise_for_status()
                return response.json()
            except Exception as e:
                if attempt < retries - 1:
                    await asyncio.sleep(5)
                    continue
                self.print_session_message(email, proxy, Fore.RED, f"Lấy Danh sách Nhiệm vụ Thất bại: {Fore.YELLOW + Style.BRIGHT}{str(e)}{Style.RESET_ALL}")
    ...
    async def complete_mission(self, email: str, mission_id: str, title: str, proxy=None, retries=5):
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 401:
                    self.print_session_message(email, proxy, Fore.WHITE, f"Nhiệm vụ {title} "
                        f"{Fore.RED + Style.BRIGHT}Chưa Hoàn thành{Style.RESET_ALL}"
                        f"{Fore.MAGENTA + Style.BRIGHT} - {Style.RESET_ALL}"
                        f"{Fore.YELLOW + Style.BRIGHT}Np Token đã hết hạn{Style.RESET_ALL}"
                    )
                    return None
                response.raise_for_status()
                return response.json()
    ...
    async def send_ping(self, email: str, idx: int, browser_id: str, proxy=None, retries=5):
        ...
        for attempt in range(retries):
            try:
                response = await asyncio.to_thread(requests.post, url=url, headers=headers, data=data, proxy=proxy, timeout=60, impersonate="chrome110", verify=False)
                if response.status_code == 401:
                    self.print_ping_message(email, idx, browser_id, proxy, Fore.RED, f"PING Thất bại: {Fore.YELLOW + Style.BRIGHT}Np Token đã hết hạn{Style.RESET_ALL}")
                    return None
                elif response.status_code == 429:
                    self.print_ping_message(email, idx, browser_id, proxy, Fore.RED, f"PING Thất bại: {Fore.YELLOW + Style.BRIGHT}Quá nhiều yêu cầu, Thử lại sau 5 phút.{Style.RESET_ALL}")
                ...
                if attempt < retries - 1:
                    await asyncio.sleep(5)
                    continue
                self.print_ping_message(email, idx, browser_id, proxy, Fore.RED, f"PING Thất bại: {Fore.YELLOW + Style.BRIGHT}{str(e)}{Style.RESET_ALL}")
    ...
    async def process_auth_session(self, email: str, use_proxy: bool, rotate_proxy: bool):
        while True:
            ...
            if session and session.get("msg") == "Success":
                season_name = session.get("data", {}).get("balance", {}).get("season_name") or "Mùa #N/A"
                ...
                self.print_session_message(email, proxy, Fore.GREEN, f"{season_name}"
                    f"{Fore.MAGENTA + Style.BRIGHT} - {Style.RESET_ALL}"
                    f"{Fore.CYAN + Style.BRIGHT}Thu nhập Hôm nay:{Style.RESET_ALL}"
                    f"{Fore.WHITE + Style.BRIGHT} {current_amount} PTS {Style.RESET_ALL}"
                    f"{Fore.MAGENTA + Style.BRIGHT}-{Style.RESET_ALL}"
                    f"{Fore.CYAN + Style.BRIGHT} Tổng Thu nhập: {Style.RESET_ALL}"
                            ...
                            if status == "AVAILABLE":
                                ...
                                if complete and complete.get("msg") == "Success":
                                    self.print_session_message(email, proxy, Fore.WHITE, f"Nhiệm vụ {title}"
                                        ...
                                        f"{Fore.MAGENTA + Style.BRIGHT}-{Style.RESET_ALL}"
                                        f"{Fore.CYAN + Style.BRIGHT} Thưởng: {Style.RESET_ALL}"
    ...
    async def process_send_ping(self, email: str, idx: int, browser_id: str, use_proxy: bool, rotate_proxy: bool):
        while True:
            ...
            print(
                ...
                f"{Fore.BLUE + Style.BRIGHT}Đang thử gửi Ping...{Style.RESET_ALL}                                   ",
                end="\r",
                flush=True
            )
            ...
            while True:
                ...
                if ping and ping.get("msg") == "Success":
                    ...
                    self.print_ping_message(email, idx, browser_id, proxy, Fore.GREEN, "PING Thành công "
                        f"{Fore.MAGENTA + Style.BRIGHT}-{Style.RESET_ALL}"
                        f"{Fore.CYAN + Style.BRIGHT} Điểm IP: {Style.RESET_ALL}"
            ...
            print(
                f"{Fore.CYAN + Style.BRIGHT}[ {datetime.now().astimezone(wib).strftime('%x %X %Z')} ]{Style.RESET_ALL}"
                f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                f"{Fore.BLUE + Style.BRIGHT}Chờ 55 phút cho Ping tiếp theo...{Style.RESET_ALL}",
                end="\r"
            )
                ...
                if account:
                    ...
                    if not user_id or not exp_time:
                        self.log(
                            ...
                            f"{Fore.MAGENTA + Style.BRIGHT} - {Style.RESET_ALL}"
                            f"{Fore.CYAN + Style.BRIGHT}Trạng thái:{Style.RESET_ALL}"
                            f"{Fore.RED + Style.BRIGHT} Np Token không hợp lệ {Style.RESET_ALL}"
                            f"{Fore.CYAN + Style.BRIGHT}]{Style.RESET_ALL}"
                        )
                        continue
                    ...
                    if int(time.time()) > exp_time:
                        self.log(
                            ...
                            f"{Fore.MAGENTA + Style.BRIGHT} - {Style.RESET_ALL}"
                            f"{Fore.CYAN + Style.BRIGHT}Trạng thái:{Style.RESET_ALL}"
                            f"{Fore.RED + Style.BRIGHT} Np Token đã hết hạn {Style.RESET_ALL}"
                            f"{Fore.CYAN + Style.BRIGHT}]{Style.RESET_ALL}"
                        )
                        continue
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"{Fore.CYAN + Style.BRIGHT}[ {datetime.now().astimezone(wib).strftime('%x %X %Z')} ]{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
            f"{Fore.RED + Style.BRIGHT}[ THOÁT ] Nodepay - BOT{Style.RESET_ALL}                                       "                              
        ) 